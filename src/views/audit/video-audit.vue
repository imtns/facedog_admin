    ...
    <div class="app-container">
        ...
        <el-dialog v-el-drag-dialog width="30%" :visible.sync="dialogTooltip" :title="dialogtt" @dragDialog="handleDrag">
            ...
            <div class="text-center" style="padding-top:20px">
                ...
                <el-button type="primary" @click="operation(tooltipID, tooltipStatus,tooltipLevel)">确定</el-button>
            </div>
        </el-dialog>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    showTooltip(id, status, level) {
      this.dialogtt = "警告";
      this.tooltipStatus = status;
      (this.tooltipLevel = level), (this.tooltipID = id);
    ...
    operation(user_id, status, level) {
      ...
      handleVideo(user_id, status, level).then(() => {
        setTimeout(() => {
          this.$message({
            message: "操作成功",
            type: "success",

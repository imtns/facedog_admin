<template>
    <div class="app-container">
        <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
            <el-table-column label="被举报人" align="center" width="200">
                <template slot-scope="scope">
                    <router-link :to="'/users/'+scope.row.report_user_id" class="link-type">
                        <span>{{scope.row.report_username}}</span>
                    </router-link>
                </template>
            </el-table-column>
        ...
        <el-dialog v-el-drag-dialog :visible.sync="dialogTableVisible" :title="dialogTitle" @dragDialog="handleDrag">
            <el-form>
                <el-form-item v-if="!status" label="处理意见">
                    <el-input v-model="opinion" :autosize="{ minRows: 2, maxRows: 4}" type="textarea" placeholder="" />
                    <el-row type="flex" class="row-bg" justify="center" style="padding-top: 20px">
                        <el-col :span="6">
                            <el-button v-waves type="primary" @click="handelReports">提交</el-button>
                        </el-col>
                    </el-row>
                </el-form-item>
            </el-form>
...
<script>
import { get_userReports } from "@/api/guider";
...
export default {
  ...
  methods: {
    getList() {
      ...
      get_userReports(this.listQuery).then((response) => {
